...
static bool lua_timer_loaded = false;
...
static void timer_callback(void* arg) {
    int id = (int)arg;
    remove_timer_item(id); //it is not necessary any more can be removed to be place for another timer if is creaked in callback

    if (!lua_timer_loaded) {
        if ( luaL_loadfile(sL, "/lua/timer.lua")) { /* Load but don't run the Lua script */
            ESP_LOGE(TAG, "timer_int load file failed: %s\n", lua_tostring(sL, -1));
        }
        lua_timer_loaded = true;
    }
...
static int cl_start_timer_once(lua_State *L) {
    LUA_PARAM_NR_CHECK(2);
    int32_t timer_id = lua_tointeger(L, 1);
    int32_t time_us = lua_tointeger(L, 2);

    lua_pushinteger(L, hugo_start_timer_once(timer_id, time_us));
    return 1;
}
...
void hugo_timer_init_module(lua_State* L) {
    ...
    REGISTER_LUA_FUNCTION(L, cl_start_timer_once);
}
