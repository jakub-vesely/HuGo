...
class SoundBlock(BlockBase):
  ...
  def play_by_name(self, name: int):
    return self._send_and_receive(_play_by_root_name_command_id, name, 1)

  def _process_folder_name(self, folder_name:str):
    length = len(folder_name)
    if length > 5:
      logging.error("folder name is longer than allowed 5 chars")
      return None
    if length < 5:
  ...
  def play_by_folder_index(self, folder_name:str, index: int):
    ...
    """
    data = self._process_folder_name(folder_name)
    if not data:
      return None
  ...
  def get_track_count(self) -> int:
    data = self._send_and_receive(_get_track_count_command_id, None, 2)
    if data and data[0] == _get_track_count_command_id:
      return data[1]
    return None

  def _get_folder_track_count_command_id(self, folder_name) -> int:
    in_data = self._process_folder_name(folder_name)
    if not in_data:
      return None

    out_data = self._send_and_receive(_get_folder_track_count_command_id, in_data, 2)
    if out_data and out_data[0] == _get_folder_track_count_command_id:
      return out_data[1]
    return None

  def _get_track_name_command_id(self) -> str:
