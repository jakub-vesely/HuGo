...
static HugoTinyBlockBaseArduino blockBase;
...
void i2c_receive_data(int count) {
  if (count > 0){
    s_data = 0; //for not-implemented cases
    uint8_t command = Wire.read();
    switch (command){
    case I2C_COMMAND_GET_CHARGING_STATE:
       s_data = (digitalRead(CHARGE_STATE_PIN) == HIGH ? LOW : HIGH); //pin is set to LOW when charging
      break;

    case I2C_COMMAND_SET_INA_A0A1:
        if (count > 1){
          s_ina_a = Wire.read();
          EEPROM.write(EEPROM_A0A1_POS, s_ina_a);
        }
      break;
     case I2C_COMMAND_GET_INA_I2C_ADDRESS:
      s_data = _get_ina_address();
      break;

    default:
      blockBase.ReceiveBaseI2cData(command, count - 1, &s_data);
    }
  }
}
...
void setup(){
  ...
  Wire.begin(blockBase.GetI2cAddress());
