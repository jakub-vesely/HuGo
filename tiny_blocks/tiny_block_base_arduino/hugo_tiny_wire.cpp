    ...
    if (block_id == s_block_type_id){
        ...
        switch (command){
            case I2C_COMMAND_GET_EXT_COUNT:
                s_buffer.data[0] = 0;
                for index = 0; index < sizeof(s_ext_addresses); ++index:
            ...
            case I2C_COMMAND_SET_EXT_ADDRESS:{
                uint8_t index = Wire.read();
                uint8_t new_address = Wire.read();
                s_buffer.data[0] = HugoTinyWireSetExtAddress(index, new_address);
                s_buffer.size = 1;
            }
            break;
            ...
            case I2C_COMMAND_GET_EXT_ADDR_LIST:{
                ...
                uint8_t count = HugoTinyWireRead();
                for (index = 0; index < sizeof(s_ext_addresses); ++index){
                    if (count == 0){
                        if (s_ext_addresses[index] == 0x00){
                            break;
